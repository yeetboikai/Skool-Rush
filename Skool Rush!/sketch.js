...
function preload() {
  ...
  map = loadImage('assets/first_floor1.png');
}
...
function draw(){
  ...
  fill(255,0,0);
  textSize(96);
  text(round(30-(frameCount-timer)/20),camera.position.x-100,camera.position.y-400);
    b.drawPlayer();
    b.movePlayer();
    b.detectWall();
}
...
movePlayer(){
  if (keyIsDown(LEFT_ARROW) && testLeft[0]>=50) {
    this.x = this.x-2;
    movement = "left";
  } else if (keyIsDown(RIGHT_ARROW) && testRight[0]>=50) {
    this.x = this.x+2;
    movement = "right";
  }else{
    movement = "standingfb";
  ...
  if (keyIsDown(UP_ARROW) && testUp[0]>=50) {
    this.y = this.y-2;
    movement = "forward";
  }else if (keyIsDown(DOWN_ARROW) && testDown[0]>=50) {
    this.y = this.y+2;
    movement = "backward";
  }
}
